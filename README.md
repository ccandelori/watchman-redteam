...
Compare a current run against a baseline. By default, the command exits nonzero when regressions are detected. Add `--strict` when a CI gate should require an exact baseline match and also fail on improvements, new scenarios, or missing baseline scenarios.
...
Baseline promotion validates the source JSONL, creates parent directories, canonicalizes volatile top-level and nested runtime fields for committed baselines, strips raw responses, preserves detector/policy summaries, and refuses to overwrite an existing baseline unless `--force` is passed. Campaign compare rejects duplicate scenario names and exits nonzero when regressions are detected; add `--strict` to fail on any baseline difference, including improvements, new scenarios, or missing baseline scenarios.
